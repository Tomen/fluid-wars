...
export class GameSimulator {
    ...
    applyAIAction(player, action) {
        ...
        player.moveCursorTowards(targetX, targetY, this.config.fixedDt, this.config.canvasWidth, this.config.canvasHeight);
    }
    /**
    ...
    getState() {
        ...
        const particleCounts = players.map(p => totalParticles > 0 ? p.particleCount / totalParticles : 0);
        // Normalize time
     ...
     */
    computeObstacleGrid(obstacles, cellWidth, cellHeight) {
        const { gridRows, gridCols } = this.config;
