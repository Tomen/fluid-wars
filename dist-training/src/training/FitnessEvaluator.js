...
export class FitnessEvaluator {
    config;
    playerCount;
    matchResults = []; // Collect all match results
    constructor(config = {}) {
    ...
    getGenerationStats() {
        ...
        if (n === 0) {
            return {
                ...
                avgCursorTargets: new Array(this.playerCount).fill([0, 0]),
        ...
        return {
            ...
            avgCursorTargets: totalTargetsX.map((x, i) => [x / n, totalTargetsY[i] / n]),
     ...
     */
    async evaluateModel(model, population) {
        let totalScore = 0;
        let totalTargetX = 0;
        let totalTargetY = 0;
        let totalVariance = 0;
    ...
    async playMatch(models) {
        ...
        const ais = models.map((model, i) => new NeuralAI(i, model, this.config.encoderConfig));
        // Reset simulator
        simulator.reset();
        ...
        const avgCursorTargets = allTargets.map(targets => {
            if (targets.length === 0)
                return [0, 0];
        ...
        const cursorVariance = allTargets.map((targets, i) => {
            if (targets.length === 0)
                return 0;
            const [avgX, avgY] = avgCursorTargets[i];
            return targets.reduce((s, t) => s + (t[0] - avgX) ** 2 + (t[1] - avgY) ** 2, 0) / targets.length;
        });
        // Get final state
        const game = simulator.getGame();
        ...
        const scores = players.map((_, i) => this.calculateScore(i, winner, steps, finalParticleCounts, conversions, startingParticles));
        // Cleanup
        simulator.destroy();
        return {
            winner,
     ...
     */
    calculateScore(playerId, winner, steps, particleCounts, conversions, startingParticles) {
     ...
     */
    sampleOpponents(opponents, count) {
     ...
     */
    async playDiagnosticMatch(models) {
