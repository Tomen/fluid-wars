...
export class ObservationEncoder {
     ...
     * @returns 3D grid observation
     */
    encodeFromRawData(playerId, particles, obstacles, canvasWidth, canvasHeight) {
     ...
     */
    buildGridFromRaw(particles, obstacles, playerId, cellWidth, cellHeight) {
        const { gridRows, gridCols, channels, maxDensity } = this.config;
        // Initialize grid with zeros
        const grid = [];
        for (let r = 0; r < gridRows; r++) {
     ...
     */
    computeObstacleGridFromRaw(obstacles, cellWidth, cellHeight) {
        const { gridRows, gridCols } = this.config;
        // Initialize grid with zeros
        const grid = [];
        for (let r = 0; r < gridRows; r++) {
